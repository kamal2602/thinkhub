/*
  # Multiple Internal Asset IDs System

  1. New Tables
    - `asset_internal_ids`
      - `id` (uuid, primary key)
      - `asset_id` (uuid, foreign key to assets)
      - `internal_id` (text, unique) - the barcode value
      - `is_primary` (boolean) - first ID added
  ...
  2. Changes
    - Keep `assets.internal_asset_id` for backward compatibility
    - All internal IDs stored in `asset_internal_ids` table
...
CREATE OR REPLACE FUNCTION set_added_by()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS set_added_by_trigger ON asset_internal_ids;
CREATE TRIGGER set_added_by_trigger
  BEFORE INSERT ON asset_internal_ids
  FOR EACH ROW
  EXECUTE FUNCTION set_added_by();

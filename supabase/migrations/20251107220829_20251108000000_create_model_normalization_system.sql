/*
  ...
  2. New Tables
    - `model_aliases` - Maps variant model names to canonical/standard names
      ...
      - `brand` (text) - Brand name (HP, Dell, Lenovo, etc.)
      - `variant_name` (text) - Various ways model appears in imports
      - `canonical_name` (text) - Standardized model name
      - `full_model_name` (text) - Complete name with brand prefix
      - `confidence_score` (integer) - Auto-learning confidence (0-100)
      - `created_by` (uuid) - User who created/approved this mapping
...
-- Insert common HP EliteBook variants as examples
INSERT INTO model_aliases (company_id, brand, variant_name, canonical_name, full_model_name, confidence_score, notes)
SELECT 
  c.id,
  'HP',
  variant,
  ...
  100,
  'Pre-populated common variant'
FROM companies c,
LATERAL (VALUES 
  ('840 G10'),
  ('Elitebook 840 G10'),
  ('EliteBook 840 G10'),
  ('EliteBook 840 G10 Notebook PC'),
  ('HP EliteBook 840 G10'),
  ('HP EliteBook 840 G10 Notebook PC'),
  ('HP 840 G10')
) AS variants(variant)
WHERE EXISTS (SELECT 1 FROM companies WHERE id = c.id)

/*
  # Create component_market_prices table
  ...
  2. Security
    - Enable RLS on component_market_prices table
    - Add policy for viewing prices in user's companies
    - Add policy for staff+ to manage prices

  3. Functions
    - Price history tracking trigger
    - Price suggestion function
    - Lot value calculation function
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE harvested_components_inventory
      ...
      ADD COLUMN harvested_by uuid REFERENCES profiles(id);
  END IF;
END $$;
